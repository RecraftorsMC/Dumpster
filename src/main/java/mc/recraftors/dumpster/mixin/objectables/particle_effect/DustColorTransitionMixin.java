...
public abstract class DustColorTransitionMixin extends AbstractDustParticleEffect implements IObjectable {
    @Shadow public abstract ParticleType<DustColorTransitionParticleEffect> getType();

    @Shadow @Final private Vec3f toColor;

    DustColorTransitionMixin(Vec3f color, float scale) {
        super(color, scale);
    }
