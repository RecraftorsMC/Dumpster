...
public final class ServerDumpCommand {
        ...
        LiteralArgumentBuilder<ServerCommandSource> builder = literal("dump")
                ...
                .then(literal("data")
                        ...
                        .then(literal("worldgen")
                                ...
                                .then(literal("biomes").executes(c -> dumpWorldgen(c, T, F, F, F, F, F)))
                                .then(literal("configured-carvers").executes(c -> dumpWorldgen(c, F, T, F, F, F, F)))
                                .then(literal("configured-features").executes(c -> dumpWorldgen(c, F, F, T, F, F, F)))
                                .then(literal("density-functions").executes(c -> dumpWorldgen(c, F, F, F, T, F, F)))
                                .then(literal("flat-generator-presets").executes(c -> dumpWorldgen(c, F, F, F, F, T, F)))
                                .then(literal("noise").executes(c -> dumpWorldgen(c, F, F, F, F, F, T)))
                        )
                )
    ...
    private static int dumpWorldgen(CommandContext<ServerCommandSource> context, boolean b1, boolean b2,
                                    boolean b3, boolean b4, boolean b5, boolean b6) {
        return dumpData(context, new DumpCall.Data(F,F,F,F,F,F,F,F,T, new DumpCall.Worldgen(b1, b2, b3, b4, b5, b6, F,F,F,F,F)));
    }
